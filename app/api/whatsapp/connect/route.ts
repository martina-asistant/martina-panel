...
const WABA_ID = "1781228559216594"; // ramblavilardental — confirmado en WhatsApp Manager
...
export async function POST(request: Request) {
  try {
    ...
    const sessionPhoneNumberId = body.phone_number_id?.trim();
    const sessionBusinessId = body.business_id?.trim() || null;
    ...
    // FIX: aviso en lugar de rechazo duro — con una sola clínica no debe
    // bloquear el flujo, solo alertarnos si alguna vez cambia.
    if (sessionWabaId !== WABA_ID) {
      console.warn("⚠️ WABA distinta a la esperada — revisar:", {
        esperada: WABA_ID,
        recibida: sessionWabaId,
      });
    }
    ...
    // 1. Obtener los números asociados a la WABA recibida (no la fija).
    const wabaIdParaConsultar = sessionWabaId || WABA_ID;

    const phoneNumbersUrl = new URL(
      `https://graph.facebook.com/${GRAPH_VERSION}/${wabaIdParaConsultar}/phone_numbers`
    );
    ...
    const phoneNumbersResponse = await fetch(phoneNumbersUrl.toString(), {
      method: "GET",
    ...
    if (!phoneNumbersResponse.ok || phoneNumbersData.error) {
      const errorMessage = getMetaErrorMessage(phoneNumbersData.error);
      await marcarWhatsappError({ ultimo_error: errorMessage });
      return NextResponse.json(
        {
          ok: false,
    ...
    const phoneNumber = (phoneNumbersData.data || []).find(
      (phone) =>
        ...
        normalizarTelefono(phone.display_phone_number) === TELEFONO_ESPERADO
    );
      ...
      return NextResponse.json(
        {
          ...
          error: errorMessage,
          debug: {
            received_phone_number_id: sessionPhoneNumberId,
            expected_phone: TELEFONO_ESPERADO,
    ...
    const subscribeUrl = `https://graph.facebook.com/${GRAPH_VERSION}/${wabaIdParaConsultar}/subscribed_apps`;
    ...
    if (!subscribeResponse.ok || subscribeData.error) {
      const errorMessage = getMetaErrorMessage(subscribeData.error);
      await marcarWhatsappError({ ultimo_error: errorMessage });
      return NextResponse.json(
        {
          ok: false,
    ...
    return NextResponse.json({
      ...
      display_phone_number: integracion.display_phone_number,
      ...
      debug: {
        ...
        session_waba_matches: sessionWabaId === WABA_ID,
        session_phone_number_matches: true,
      },
    });
  } catch (error) {
    const errorMessage =
      error instanceof Error ? error.message : "Error interno conectando WhatsApp";
    ...
    try {
      ...
      console.error("No se pudo guardar el error en Supabase:", supabaseError);
    }
