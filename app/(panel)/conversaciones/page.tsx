...
import { useEffect, useMemo, useState } from 'react';import { Input } from '@/components/ui/input';import { Button } from '@/components/ui/button';import { Search, Paperclip, Send, Save } from 'lucide-react';import { cn } from '@/lib/utils/cn';import {listConversaciones,tomarConversacion,devolverAMartina,cerrarGestion,actualizarNotasConversacion} from '@/lib/repos/conversaciones.repo';import { listMensajesByConversation } from '@/lib/repos/mensajes.repo';import { getPatientById, updatePatientNotas } from '@/lib/repos/patients.repo';import type {ConversacionWhatsapp,MensajeWhatsapp,Patient,EstadoVisualConv} from '@/lib/types/db.types';import { createClient, isSupabaseConfigured } from '@/lib/supabase/client';import {formatRelativeOrTime,formatTime,lastActivity,formatDate} from '@/lib/utils/formatDate';import { conversacionLabel } from '@/lib/utils/visualMaps';import { Textarea } from '@/components/ui/textarea';import { toast } from 'sonner';

type Filtro = 'todas' | EstadoVisualConv;

const filtros: { key: Filtro; label: string }[] = [{ key: 'todas', label: 'Todas' },{ key: 'nueva', label: 'Martina activa' },{ key: 'recepcion', label: 'Recepción' },{ key: 'gestionada', label: 'Gestionadas' },];

const formatTelefono = (telefono?: string | null) => {if (!telefono) return '';
...
const ConversacionesView = () => {const [convs, setConvs] = useState<ConversacionWhatsapp[]>([]);const [selectedId, setSelectedId] = useState<string | null>(null);const [mensajes, setMensajes] = useState<MensajeWhatsapp[]>([]);const [paciente, setPaciente] = useState<Patient | null>(null);const [filter, setFilter] = useState('todas');const [search, setSearch] = useState('');const [notasPaciente, setNotasPaciente] = useState('');const [notasConv, setNotasConv] = useState('');const [userEmail, setUserEmail] = useState('demo@martina.local');

const selected = useMemo(() => convs.find(c => c.id === selectedId) || null,[convs, selectedId]);

useEffect(() => {(async () => {const list = await listConversaciones();setConvs(list);if (list[0]) setSelectedId(list[0].id);})();
...
useEffect(() => {const supa = createClient();if (!supa) return;
...
useEffect(() => {if (!selectedId) {setMensajes([]);setPaciente(null);return;}
...
useEffect(() => {const supa = createClient();if (!supa || !selectedId) return;
...
const filtered = convs.filter(c => {if (filter !== 'todas' && c.estado_visual !== filter) return false;
...
const doCerrar = async () => {if (!selected) return;await cerrarGestion(selected.id);const updated = await listConversaciones();

// quitar la conversación cerrada de la vista actualconst visibles = updated.filter(c => c.estado_visual !== 'gestionada');

setConvs(visibles);

// volver a estado basesetSelectedId(visibles[0]?.id || null);
...
{filtros.filter(f => f.key === 'todas').map(f => (<buttonkey={f.key}onClick={() => setFilter(f.key)}className={cn('text-[12px] px-2.5 py-[5px] rounded-full border transition-colors whitespace-nowrap',filter === f.key? 'bg-cyan-500/20 text-cyan-200 border-cyan-300/50 shadow-[0_0_18px_rgba(34,211,238,.22)]': 'bg-white/5 text-cyan-100/65 border-cyan-500/20 hover/10 hover')}>{f.label}))}
...
<buttononClick={doCerrar}className="absolute top-2 right-2 w-7 h-7 rounded-full flex items-center justify-center text-white bg-[linear-gradient(180deg,#214955_0%,#163C46_100%)] border border-cyan-300/15 shadow-[0_0_0_3px_rgba(34,211,238,.10),0_0_18px_rgba(34,211,238,.25)] hover transition-all z-20"
...
const isPaciente =m.tipo_emisor === 'paciente' ||m.direccion === 'entrante';

return (<divkey={m.id}className={cn('flex',isPaciente ? 'justify-start' : 'justify-end')}><divclassName={cn('max-w-[72%] rounded-2xl px-5 py-3 text-sm shadow-sm',isPaciente? 'bg-white border border-slate-200 text-[#06111A] rounded-bl-sm': 'bg-[#D9F7FA] border border-[#B6EAEF] text-[#184B53] rounded-br-sm shadow-[0_0_12px_rgba(34,211,238,.08)]')}>{m.contenido_texto || ''}
